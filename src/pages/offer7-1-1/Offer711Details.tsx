...
const Offer711Details = () => {
  ...
  useEffect(() => {
    if (state?.mode === "view" || state?.mode === "edit") {
      ...
      setFormData({
        ...
        description: "Specialized medical equipment rental offer for subservice 7.1.1 with advanced features",
        discount: "25",
        startDate: "2024-01-01",
        endDate: "2024-12-31",
        status: "Active",
        terms: "Valid for specialized medical facilities",
        conditions: "Minimum rental period of 60 days required for subservice 7.1.1"
      });
    }
  }, [state]);
  ...
  const handleBack = () => {
    navigate("/offer7-1-1/dashboard", { state });
  };
          ...
          <header className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="max-w-xl text-sm font-textMedium text-gray-500">
                {state?.category?.title || "Category 7"} - {state?.service?.title || "Service 1"} - Subservice 7.1.1
              </p>
            </div>
            <button
              onClick={handleBack}
...
export default Offer711Details;
