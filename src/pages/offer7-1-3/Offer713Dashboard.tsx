...
const Offer713Dashboard = () => {
  ...
  useEffect(() => {
    // Initialize offers data for subservice 7.1.3
    setOffers([
      {
        id: 1,
        offerNo: "#OFF713",
        title: "Medical Equipment Rental - Subservice 7.1.3",
        description: "Specialized medical equipment rental offer for subservice 7.1.3",
        status: "Active",
        ...
        subService: "7.1.3 - " + (state?.subService?.title || "Sub-service 3")
      }
    ]);
  }, [state]);

  const handleCreateOffer = () => {
    navigate("/offer7-1-3/details/new", {
      state: {
        ...state,
        mode: "create"
      }
  ...
  const handleViewOffer = (offerId: number) => {
    navigate(`/offer7-1-3/details/${offerId}`, {
      state: {
        ...state,
        mode: "view",
        offerId
          ...
          <header className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-2">
              <h1 className="text-3xl font-helveticaBold text-primary">
                Subservice 7.1.3 - Offers
              </h1>
              <p className="max-w-xl text-sm font-textMedium text-gray-500">
                Manage offers for {state?.category?.title || "Category 7"} - {state?.service?.title || "Service 1"} - Subservice 7.1.3
              </p>
            </div>
            <button
                ...
                {offers.map((offer) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="grid h-12 w-12 place-items-center rounded-xl bg-emerald-100">
...
export default Offer713Dashboard;
