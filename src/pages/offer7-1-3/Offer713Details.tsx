...
const Offer713Details = () => {
  ...
  useEffect(() => {
    if (state?.mode === "view" || state?.mode === "edit") {
      ...
      setFormData({
        ...
        description: "Specialized medical equipment rental offer for subservice 7.1.3 with advanced medical features",
        discount: "35",
        startDate: "2024-01-01",
        endDate: "2024-12-31",
        status: "Active",
        terms: "Valid for advanced medical facilities and hospitals",
        conditions: "Minimum rental period of 120 days required for subservice 7.1.3"
      });
    }
  }, [state]);
  ...
  const handleBack = () => {
    navigate("/offer7-1-3/dashboard", { state });
  };
          ...
          <header className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="max-w-xl text-sm font-textMedium text-gray-500">
                {state?.category?.title || "Category 7"} - {state?.service?.title || "Service 1"} - Subservice 7.1.3
              </p>
            </div>
            <button
              onClick={handleBack}
...
export default Offer713Details;
