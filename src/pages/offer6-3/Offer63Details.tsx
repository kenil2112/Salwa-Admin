...
const Offer63Details = () => {
  ...
  useEffect(() => {
    if (state?.mode === "view" || state?.mode === "edit") {
      ...
      setFormData({
        ...
        endDate: "2024-12-31",
        status: "Active",
        terms: "Valid for VIP customers only",
        conditions: "Minimum service duration of 12 months required"
      });
    }
  }, [state]);
  ...
  const handleBack = () => {
    navigate("/offer6-3/dashboard", { state });
  };
          ...
          <header className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="max-w-xl text-sm font-textMedium text-gray-500">
                {state?.category?.title || "Category 6"} - {state?.service?.title || "Service 3"}
              </p>
            </div>
            <button
              onClick={handleBack}
