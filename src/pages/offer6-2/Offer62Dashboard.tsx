...
const Offer62Dashboard = () => {
    ...
    setOffers([
      {
        id: 1,
        offerNo: "#OFF002",
        title: "Service Package Offer",
        description: "Special package deal for comprehensive services",
        status: "Active",
        startDate: "2024-01-01",
        endDate: "2024-12-31",
        discount: "15%",
        category: state?.category?.title || "Category 6",
        service: state?.service?.title || "Service 2"
      }
    ]);
  }, [state]);

  const handleCreateOffer = () => {
    navigate("/offer6-2/details/new", {
      state: {
        ...state,
        mode: "create"
      }
  ...
  const handleViewOffer = (offerId: number) => {
    navigate(`/offer6-2/details/${offerId}`, {
      state: {
        ...state,
        mode: "view",
        offerId
          ...
          <header className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-2">
              <h1 className="text-3xl font-helveticaBold text-primary">
                {state?.service?.title || "Service 2"} - Offers
              </h1>
              <p className="max-w-xl text-sm font-textMedium text-gray-500">
                Manage offers for {state?.category?.title || "Category 6"} - {state?.service?.title || "Service 2"}
              </p>
            </div>
            <button
                ...
                {offers.map((offer) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="grid h-12 w-12 place-items-center rounded-xl bg-blue-100">
...
export default Offer62Dashboard;
