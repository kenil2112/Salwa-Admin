...
const Offer91Dashboard = () => {
    ...
    setOffers([
      {
        ...
        endDate: "2024-12-31",
        discount: "35%",
        category: state?.category?.title || "Category 9",
  ...
  const handleViewOffer = (offerId: number) => {
    navigate(`/offer9-1/details/${offerId}`, {
      state: {
        ...state,
        mode: "view",
        offerId
          ...
          <header className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="max-w-xl text-sm font-textMedium text-gray-500">
                Manage offers for {state?.category?.title || "Category 9"} - {state?.service?.title || "Service 1"}
              </p>
            </div>
            <button
                ...
                {offers.map((offer) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="grid h-12 w-12 place-items-center rounded-xl bg-indigo-100">
...
export default Offer91Dashboard;
