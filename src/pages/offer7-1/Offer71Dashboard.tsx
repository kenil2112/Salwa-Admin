...
const Offer71Dashboard = () => {
    ...
    setOffers([
      {
        id: 1,
        offerNo: "#OFF071",
        title: "Medical Equipment Rental Offer",
        description: "Special offer on medical equipment rental with premium support",
        status: "Active",
        ...
        subService: state?.subService?.title || "Sub-service"
      }
    ]);
  }, [state]);

  const handleCreateOffer = () => {
    navigate("/offer7-1/details/new", {
      state: {
        ...state,
        mode: "create"
      }
  ...
  const handleViewOffer = (offerId: number) => {
    navigate(`/offer7-1/details/${offerId}`, {
      state: {
        ...state,
        mode: "view",
        offerId
          ...
          <header className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-2">
              <h1 className="text-3xl font-helveticaBold text-primary">
                {state?.service?.title || "Service 1"} - Offers
              </h1>
              <p className="max-w-xl text-sm font-textMedium text-gray-500">
                Manage offers for {state?.category?.title || "Category 7"} - {state?.service?.title || "Service 1"}
                {state?.subService && ` - ${state.subService.title}`}
              </p>
            </div>
            <button
                ...
                {offers.map((offer) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="grid h-12 w-12 place-items-center rounded-xl bg-orange-100">
...
export default Offer71Dashboard;
